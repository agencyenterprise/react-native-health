            ...
            for(HKSample * sample in results) {
              if(sample){
                NSObject*metaData = [sample metadata] ? [sample metadata] : @{};
                  NSString *deviceName = [sample metadata] ? [metaData valueForKey:@"HKDeviceName"] : ([bundleId hasPrefix:@"com.handa.dafit"] ? @"Smart Watch":@"");
                NSString *manufacturer = [sample metadata] ? [metaData valueForKey:@"HKDeviceManufacturerName"] : @"";
                NSString *serialNumber = [sample metadata] ? [metaData valueForKey:@"HKDeviceSerialNumber"] : @"";
                //                NSString *deviceType = [metaData valueForKey:@"HKDeviceName"];
                NSString *tempDeviceKey=[[[[sample sourceRevision] source] name] stringByAppendingFormat:@"/%@", @"Watch"];
                if([deviceName length]!=0 && [serialNumber length]!=0)
                  tempDeviceKey=[deviceName stringByAppendingFormat:@"/%@", serialNumber];
...
//                      double value = [quantity doubleValueForUnit:unit];
                    ...
                    if([sample sampleType]==[HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierHeartRate]) {
                      ...
                      HKUnit *unitHeartRate = [count unitDividedByUnit:minute];
                       
                      double value = [quantity doubleValueForUnit:unitHeartRate];
                      ...
                      HKUnit *unitBloodPressure = [HKUnit millimeterOfMercuryUnit];
                      ...
                      [elem setValue:@([bloodPressureSystolicValue.quantity doubleValueForUnit:unitBloodPressure]) forKey:@"bloodPressureSystolicValue"];
                      [elem setValue:@([bloodPressureDiastolicValue.quantity doubleValueForUnit:unitBloodPressure]) forKey:@"bloodPressureDiastolicValue"];
