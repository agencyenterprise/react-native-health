...
- (void)dietary_getEnergyConsumedSamples:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    HKUnit *unit = [self.rnAppleHealthKit hkUnitFrom:input with:@"unit" defaultValue:[HKUnit kilocalorieUnit]];
...
- (void)saveFood:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    NSDate *timeFoodWasConsumed = [self.rnAppleHealthKit dateFrom:input key:@"date" defaultDate:[NSDate date]];
...
{
    NSDate *timeWaterWasConsumed = [self.rnAppleHealthKit dateFrom:input key:@"date" defaultDate:[NSDate date]];
